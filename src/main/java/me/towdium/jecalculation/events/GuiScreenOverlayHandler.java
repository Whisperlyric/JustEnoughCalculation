...
public class GuiScreenOverlayHandler extends WContainer implements IGui {
    ...
    public void setupOverlay(Inventory inventory) {
        ...
        for (int i = 0; i < widgets.size(); i++) {
            ...
            widget.setWindowCloseListener((w) -> setupOverlay(inventory));
            add(widget);
        }
    }
    ...
    public List<GuiCraftMini> inventoryToWidgets(Inventory inventory) {
        ...
        // Items removed - overlay is now purely client-side via keybind
    ...
    @Override
    public boolean onDraw(@NotNull JecaGui gui, int mouseX, int mouseY) {
        ...
        if (!Comparators.isInOrder(windows, Comparator.comparingInt(guiCraftMini -> guiCraftMini != null ? guiCraftMini.getDepth() : 0))) {
    ...
    public void onVisible(JecaGui gui) {
        IGui.super.onVisible(gui);
    }

    @Override
    public boolean acceptsTransfer() {
        return IGui.super.acceptsTransfer();
    }

    @Override
    public boolean acceptsLabel() {
        return IGui.super.acceptsLabel();
    }
