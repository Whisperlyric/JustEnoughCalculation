...
public class RecordMath implements IRecord {
    ...
    public RecordMath(CompoundTag nbt) {
        state = State.values()[nbt.getIntOr(KEY_STATE, 0)];
        operator = Operator.values()[nbt.getIntOr(KEY_OPERATOR, 0)];
        last = nbt.contains(KEY_LAST) ? new BigDecimal(nbt.getStringOr(KEY_LAST, "0")) : BigDecimal.ZERO;
        current = nbt.getStringOr(KEY_CURRENT, "");
    }
